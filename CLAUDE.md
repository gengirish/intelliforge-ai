...
- **Founder attribution lives only in the footer** ("Founded by {siteConfig.founder}"). Deliberately no founder name, bio, credentials, photo, byline, testimonials, or JSON-LD `founder` anywhere else on the site or in `lib/llms-content.ts` — don't re-add unless asked. The company origin story is `originStory` in `lib/constants.ts`.
...
- **Phone number source: a custom "Phone Number" invitee question, added in the Calendly dashboard** (Event Types → AI Strategy Call → Invitee Questions → Add — the Calendly API cannot add booking questions, it's dashboard-only). The webhook matches on any question whose text contains "phone" and normalizes the answer to E.164 (bare 10-digit numbers are assumed Indian, `+91`-prefixed). Calendly's own `text_reminder_number` field is checked as a fallback but shouldn't be relied on — it's the "text me reminders" opt-in, gated behind Calendly's **Teams plan or higher**, so it doesn't even appear on the booking form on Free/Essentials/Professional. Bookings with no phone number from either source silently skip the confirmation call (logged, not an error).
